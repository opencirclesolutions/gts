    ...
    public GiftLayout(BaseService<Integer, Gift> service, EntityModel<Gift> entityModel, FormOptions formOptions,
        ...
        addCustomField("translations", context -> {
            ...
            DetailsEditLayout<Integer, GiftTranslation, Integer, Gift> dt = new DetailsEditLayout<>(
                    giftTranslationService, getEntityModelFactory().getModel(GiftTranslation.class),
                    context.getAttributeModel(), context.isViewMode(), fo, new AttributeComparator<>("description"));
            dt.setCreateEntity(gift -> {
                GiftTranslation gt = new GiftTranslation();
                gift.addTranslation(gt);
                return gt;
            });
            dt.setRemoveEntity(Gift::removeTranslation);
